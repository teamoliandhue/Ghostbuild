...
| 3.14 | Session Kickoff added to `CLAUDE.md`. When a new chat starts with a casual greeting (`hey`, `hi`, `let's start`, etc.), ask the user for their name and the client URL, sanitize the name into a branch handle, create or switch to a per-person branch, and tell them the branch name before running any client work. Prevents parallel teammates from colliding on the same branch and makes commit attribution match the human. Skip this kickoff when the user opens with a URL, `continue`, or a client slug. |
